...
export default {
  ...
  computed: {
    objects3(){
      console.log(this.allSprints.length);
      let objects = [];
      ...
      for(var i = 0; i < this.allSprints.length; i++){
        ...
        objects.push({ x: point.x, y: point.y, radius: 4, fill: color })
      }
  ...
  methods: {
    test() {
      const vm = this;
      const path = this.$refs.trackmap._konvaNode;
      const layer = this.$refs.layer._konvaNode;

      var steps = 600; // number of steps in animation
      ...
      var frameCnt = 0, pos =0, pt;
       ...
       var anim = new Konva.Animation(function(frame) {
            ...
            vm.obstacles[0].x = pt.x;
            vm.obstacles[0].y = pt.y;
        }, layer);

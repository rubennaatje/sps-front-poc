<template>
  <div>
    ...
    <v-stage ref="stage" :config="canvas">
      <v-layer ref="layer">
        <v-path ref="trackmap" :config="pathConfig" />
      </v-layer>
      <v-layer>
        <v-circle v-for="(obstacle, i) in objects3" :key="i" :ref="'obstacle' + i" :config="obstacle" />
      </v-layer>
    </v-stage>
...
export default {
  ...
  computed: {
    objects3 () {
      ...
      for (let i = 0; i < this.allSprints.length; i++) {
  ...
  methods: {
    ...
    getPathPoint (percentage) {
      if (this.$refs.trackmap) { return this.$refs.trackmap._konvaNode.getPointAtLength(this.trackValues.percentageLength * percentage) } else { return { x: 0, y: 0 } }
    }
  }
}
